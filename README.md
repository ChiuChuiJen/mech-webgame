...
## V0.2.0 更新
- 修正：同一把武器（同一個背包物品 uid）不可同時裝備在左右手。裝備時會自動從另一手卸下/移動。

## V0.2.1 更新
- 背包「已裝備」標籤改為高亮
- 武器裝備時可選擇左手/右手
- 更新歷程 UI：預設顯示前三筆，+/- 可開啟全部歷程視窗

## V0.2.2 更新
- 武器左右手選擇視窗改暗色系
- 裝備面板新增 3 個技能槽（連動「技能」分頁）
- 新增「技能」分頁：顯示已學會技能並可裝備到 3 個技能槽
- 新增技能書掉落、商店購買；新增「大道天書」常駐商品（使用後隨機獲得技能書）
- 戰鬥技能列表：基礎技能 + 技能槽1~3（可直接施放）

## V0.2.3 更新
- 背包新增分類篩選：全部 / 武器 / 防具配件 / 道具 / 技能書
- 確認技能書/大道天書已實裝：可商店購買/掉落、背包使用後學會技能
- 戰鬥視窗：新增「下一場探索」自動開關，並可切換模式（單次/循環）

## V0.2.4 更新
- 修正：ensureSkillState 在初始載入時 S 尚未建立會造成整個 UI 崩潰（Cannot read properties of null）
- 修正：背包分類/技能分頁因上述崩潰未渲染的問題（現在可正常顯示與點擊）

## V0.2.6 更新
- 修正：ensureInvFilter / bindInvCats 在 S 尚未建立時造成崩潰（背包分類按鈕無法使用）
- 修正：技能書學習後技能分頁不更新（學習後立即刷新，並在技能分頁可見時自動重繪）
- 修正：戰鬥視窗「下一場探索」開關未顯示（加入 DOM 補救注入）

## V0.2.7 更新
- 修正：戰鬥視窗「下一場探索」旁的 單次/循環 切換無法點擊（改為每次渲染都重新綁定事件）
